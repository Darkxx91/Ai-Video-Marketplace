...
function App() {
  ...
  const handleVideoSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch(`/users/${user.id}/videos/`, {
      method: 'POST',
      headers: {
  ...
  const handleUpgrade = async () => {
    const response = await fetch(`/users/${user.id}/upgrade/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleLogin = () => {
    ...
    // You will need to implement user authentication to get the actual user.
